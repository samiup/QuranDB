...
+How to use:
- Simply run the CreateInsertScript.sql script and you should get a table with two text columns, one with tashkil signs and one without signs. Then if interested in a table of words of the Quran, run Create_Words_Table.sql.
- If you want to create the original table as per Tanzil.info, simply run:Arabic-(Original-Book)-1.sql
- If you want to do the same thing that i did (maybe better :)), then after creating the original table, alter that table and add a new text field/column called AyahTextNoTashkil and run the RemoveTashkilScript.sql script. 
...
+Satus as of 3/23/2013
Added a new script: Create_Words_Table.sql that will create a table to accomodate the words of the Quran, each row has the word itself, the (Chapter) surahID/number and (Verse)AyahID/Number that the word belongs to. All words totalled to 82600 word (needs verification against another source).  
This is not a text insert script, this scripts selects and generates a row for each word so it might take a while, i will be adding the text CreateInsert script soon.
